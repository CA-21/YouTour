...
public class StopMap extends FragmentActivity implements OnMarkerClickListener {
    ...
    public void drawLine() {
        if (LatLonPointList.size() == 1) {
            ...
            mMap.addMarker((new MarkerOptions()).position(SearchPointConvert(LatLonPointList.get(0))).icon(
                    BitmapDescriptorFactory.fromBitmap(getBitMap(1 + ""))));
            mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(SearchPointConvert(LatLonPointList.get(0)),
                    (float) (zoomLevel + 4)));
            return;
        }
        Thread t = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < LatLonPointList.size() - 1; i++) {
                    ...
                    final Route.FromAndTo fromAndTo = new Route.FromAndTo(preStop, nextStop);
                    try {
                        routeResult = Route.calculateRoute(StopMap.this, fromAndTo, mode);
                        if (routeResult != null || routeResult.size() > 0) {
                            route = routeResult.get(0);
                            if (route != null) {
                                LatLng startPoint = SearchPointConvert(route.getStartPos());
                                LatLng endPoint = SearchPointConvert(route.getTargetPos());
                                if (i == 0) {
                                    mMap.addMarker((new MarkerOptions()).position(startPoint)
                                            .icon(BitmapDescriptorFactory.fromBitmap(getBitMap((i + 1) + "")))
                                            .title(i + ""));
                                }
                                mMap.addMarker((new MarkerOptions()).position(endPoint)
                                        .icon(BitmapDescriptorFactory.fromBitmap(getBitMap((i + 2) + "")))
                                        .title((i + 1) + ""));
                                for (int i1 = 0; i1 < route.getStepCount(); i1++) {
                                    mMap.addPolyline((new PolylineOptions())
                                            .addAll(convertArrList(route.getStep(i1).getShapes()))
                                            .color(Color.argb(180, 54, 114, 227)).width(lineWidth));
                                }
                            }
                        }
                ...
                routeHandler.sendMessage(Message.obtain(routeHandler, Constants.DRAWLINE_FINISH));
            }
        });
        t.start();
